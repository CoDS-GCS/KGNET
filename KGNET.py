...
if __name__ == '__main__':
    ...
    inference_MQuery_dblp_NC_venue_aff=""" 
        ...
        where
        {
            ...
            filter(str(?Pred_aff_country) not in ("germany","china",'usa')).
            filter(?Pred_Venue not in  ("AAAI","VLDB","ACM",'CoRR')).
    ...
    Real_target_node=[str(x).replace("\"","").strip() for x in dblp_notin_AAAI_ACM_VLDB_CoRR_germany_china_usa_2021]
