...
class PygNodePropPredDataset_hsh(InMemoryDataset):
    def __init__(self, name, root = './',numofClasses=349, transform=None, pre_transform=None, meta_dict = None):
        '''
        ...
        if meta_dict is None:
            ...
            if osp.exists(self.dir_name + '_pyg'):
    ...
    def download(self):
        ...
        if str(url).startswith("http")==False:
            ...
            if self.download_name.split("/")[-1]!=self.root.split("/")[-1]:
                # shutil.rmtree(self.root)
                print(f'DEBUG: attempt to delete {self.root}. :156')
