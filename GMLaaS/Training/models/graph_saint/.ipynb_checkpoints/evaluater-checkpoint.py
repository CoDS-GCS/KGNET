...
class Evaluator:
    def __init__(self, name,p_eval_metric=None,num_tasks = 1):
        self.name = name
        ...
        self.num_tasks = num_tasks#int(meta_info[self.name]['num tasks'])
        if p_eval_metric == None:
            self.eval_metric = 'acc'#meta_info[self.name]['eval metric']
        else:
            self.eval_metric = p_eval_metric


    def _parse_and_check_input(self, input_dict):
        if self.eval_metric == 'rocauc' or self.eval_metric == 'acc' or self.eval_metric == 'cm':
    ...
    def eval(self, input_dict):
        ...
        elif self.eval_metric == 'cm':
            y_true, y_pred = self._parse_and_check_input(input_dict)
            return self._eval_cm(y_true, y_pred)
        else:
            raise ValueError('Undefined eval metric %s ' % (self.eval_metric))
    ...
    def expected_output_format(self):
        ...
        if self.eval_metric == 'rocauc':
            ...
            desc += '{\'cm\': cm}\n'
            desc += '- cm  Confusion Matrix across {} task(s)\n'.format(self.num_tasks)
        else:
            raise ValueError('Undefined eval metric %s ' % (self.eval_metric))
