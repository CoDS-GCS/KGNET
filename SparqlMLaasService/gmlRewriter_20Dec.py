...
#         string_infer_q,string_data_q,targetNodeQuery,model_ids = self.get_data_query(dict_gml_var_dict, data_triples)
#         return (string_infer_q, string_data_q,targetNodeQuery,kgmeta_model_queries_lst,model_ids)
#
...
#             return
#         if  NODECLASSIFIER in gml_operator_type:
#             for triple in gml_dict:
...
#                     dict_vars[triple["subject"]]['target'] = triple  # ['object']         # if the triple is the target type
#                     continue
#
...
#             target_variables_dict[gml_op]=target_variable
#             label_variables_dict[gml_op]=label_variable
#
#         if len(query['select'])>0:
#             string_q+='SELECT'
#             string_q_dataonly+='SELECT'
#             string_q_target_nodes+= 'SELECT distinct ?s \n'
#             lower_labels_lst= [ str(elem).lower() for elem in list(label_variables_dict.values())]
#             for item in query['select']:
#                 if item.lower() in lower_labels_lst:
#                     target_label = item.lower()+target_label_mod
...
#
#         userDefinedPredicate_lst = [self.set_syntax(elem).lower() for elem in list(userDefinedPredicate_dict.values())]
...
#             o = self.set_syntax(o)
#
#             if p.lower() in userDefinedPredicate_lst:
#                 # if p.replace('?','').lower() in self.set_syntax(userDefinedPredicate).lower():
#                 # target_type = get_rdfType(list_data_T, target_node)
#                 gml_op = list(userDefinedPredicate_dict.keys())[userDefinedPredicate_lst.index(p.lower())]
...
#                 sub_query = '{'
#                 sub_query += f"SELECT (kgnetML:{clf_fxn[gmlOperatorType_dict[gml_op].lower()]}(\"{model_uri_dict[gml_op]}\",?API_JSON) \n as {self.set_syntax(label_variables_dict[gml_op].lower())+target_label_mod} )"
#                 sub_query += ' WHERE {}} '
#                 string_q += sub_query
#                 continue
#             # s=set_syntax(s)
...
#
#         for target_variable in target_variables_dict.values():
...
#
#         return string_q ,string_q_dataonly,string_q_target_nodes,[elem.split("/")[-1] for elem in model_uri_dict.values()]
#
